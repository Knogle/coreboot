...
/*
 * X58_ACPI_PLATFORM static OS-facing descriptions. The hardware is initialized by the
 * inherited X58_IRQ_ROUTE path; none of these objects programs a register.
 *
...
Scope (\_SB)
{
	...
	Device (HPET)
	{
		...
		Name (_UID, Zero)
		/*
		 * X58_HPET HPTC=0x80, GCAP=0429b17f:8086a301, size 0x400,
		 * observed again in X58_QUIET_USB-HW-02. As in the coreboot QEMU
		...
		{
			Memory32Fixed (ReadWrite, X58_ACPI_HPET_BASE,
				X58_ACPI_HPET_SIZE, )
		})
	}
}
	...
	Device (LPCB)
	{
		...
		Device (TIMR)
		{
			...
			Name (_CRS, ResourceTemplate ()
			{
				...
				/* ISA IRQ0; the X58_IRQ_ROUTE MADT maps it to GSI2. */
				IRQNoFlags () { 0 }
			})
		}
