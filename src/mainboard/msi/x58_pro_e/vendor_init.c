...
#define X58_VENDOR_ID		0x34058086u
...
#define X58_VENDOR_MINIT_CSI_FNV1A	0x8b38506au
#define X58_VENDOR_CSI_DYNAMIC_OFFSET	0x02a6u
#define X58_VENDOR_CSI_RAW_08_FNV1A	0x03e3d24eu
#define X58_VENDOR_CSI_RAW_0C_FNV1A	0x8b38506au
#define X58_VENDOR_CSI_CANONICAL_FNV1A	0x908dabb6u
_Static_assert(X58_VENDOR_CSI_DYNAMIC_OFFSET < X58_VENDOR_CSI_STATE_SIZE,
	"X58_CSI_CANONICAL CSI canonical offset is outside the state buffer");
#define X58_VENDOR_MEMORY_RESULT_POLICY_FNV1A		0x3c0f3a0bu
#define X58_VENDOR_MEMORY_RESULT_WORKSPACE_FNV1A	0x94299f43u
#define X58_VENDOR_MEMORY_RESULT_WORK_B3_FLAGS_OFFSET	0x004fu
#define X58_VENDOR_MEMORY_RESULT_WORK_COMPLETE_OFFSET	0x0e79u
#define X58_VENDOR_MEMORY_RESULT_POLICY_STATUS_OFFSET	0x000au
#define X58_VENDOR_MEMORY_RESULT_MC_COMMON_DEV		PCI_DEV(0xff, 3, 0)
#define X58_VENDOR_MEMORY_RESULT_UNCORE_COMMON_DEV	PCI_DEV(0xff, 3, 4)
#define X58_VENDOR_MEMORY_RESULT_CHANNEL2_DEV		PCI_DEV(0xff, 6, 0)
#define X58_VENDOR_MEMORY_RESULT_CHANNEL2_ADDR_DEV	PCI_DEV(0xff, 6, 1)
...
_Static_assert(sizeof(uintptr_t) == sizeof(uint32_t),
	"X58_VENDOR_ENTRY vendor trampoline requires 32-bit romstage");
...
static uint32_t x58_csi_canonical_csi_canonical_digest(const uint8_t *state)
{
	...
	for (size_t offset = 0; offset < X58_VENDOR_CSI_STATE_SIZE; offset++) {
		const uint8_t value = offset == X58_VENDOR_CSI_DYNAMIC_OFFSET ?
			0 : state[offset];
...
static bool x58_csi_canonical_csi_digest_pair_exact(const uint8_t *state,
	uint32_t raw_digest, uint32_t canonical_digest)
{
	if (state == NULL ||
	    canonical_digest != X58_VENDOR_CSI_CANONICAL_FNV1A)
		return false;

	return (state[X58_VENDOR_CSI_DYNAMIC_OFFSET] == 0x08 &&
			raw_digest == X58_VENDOR_CSI_RAW_08_FNV1A) ||
		(state[X58_VENDOR_CSI_DYNAMIC_OFFSET] == 0x0c &&
			raw_digest == X58_VENDOR_CSI_RAW_0C_FNV1A);
}
...
static bool x58_init_phase_high_qpi_platform_for_a0(bool review_16e00)
{
	...
	if (pci_io_read_config32(X58_VENDOR_SAD_DEV, 0) != X58_VENDOR_SAD_ID ||
		...
		return false;
	if (fixed_read32(0xe0000000u) != X58_VENDOR_ID ||
	...
	return pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0x50) ==
		...
		pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0x9c) ==
			0x00000502u &&
		(review_16e00 ? x58_vendor_pre_a0_16e00_candidate(
			pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0xa0)) :
		 (x58_vendor_csi_profile_high_qpi_cpu_a0_exact(
			pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0xa0))
		  || x58_vendor_normal_stable_pre_minit_a0(
			pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0xa0))
		 )) &&
...
static bool x58_init_phase_high_qpi_platform_exact(void)
{
	return x58_init_phase_high_qpi_platform_for_a0(
		false
		);
}

static bool x58_minit_high_qpi_post_csi_platform_exact(void)
{
	const uint8_t *state;
	uint32_t raw_digest;
	...
	bool broad_profile;
	bool pre_a0_candidate;

	if (runtime.csi_state == NULL || !runtime.csi_returned)
		return false;
	state = runtime.csi_state->raw;

	/* Exact X58_VENDOR_TRACE pass-three observation; field meanings remain unknown. */
	raw_digest = bytes_digest(state, X58_VENDOR_CSI_STATE_SIZE);
	canonical_digest = x58_csi_canonical_csi_canonical_digest(state);
	...
	if (runtime.last_call.eax != 0 || runtime.last_call.ebx != 0 ||
	    ...
	    !x58_csi_canonical_csi_digest_pair_exact(state, raw_digest, canonical_digest))
		return false;
	...
	/*
	 * X58_MEMORY_PROFILE accepts the bounded CPU/CSI hard class, not a workspace profile.
	 * CSI byte/raw pairing and the complete marker tuple were checked above.
	 */
	pre_a0_candidate = x58_vendor_memory_profile_high_qpi_cpu_a0_candidate(
		runtime.x58_csi_profile_pre_csi_cpu_a0);
	pre_a0_candidate = pre_a0_candidate ||
		x58_vendor_normal_stable_pre_minit_a0(
			runtime.x58_csi_profile_pre_csi_cpu_a0);
	broad_profile = runtime.x58_csi_profile_pre_csi_cpu_a0_valid &&
		pre_a0_candidate &&
		post_cpu_a0 == runtime.x58_csi_profile_pre_csi_cpu_a0 &&
		(post_cpu_9c == 0x00a00502u || post_cpu_9c == 0x00b00502u) &&
		((state[X58_VENDOR_CSI_DYNAMIC_OFFSET] == 0x08 &&
		  raw_digest == X58_VENDOR_CSI_RAW_08_FNV1A) ||
		 (state[X58_VENDOR_CSI_DYNAMIC_OFFSET] == 0x0c &&
		  raw_digest == X58_VENDOR_CSI_RAW_0C_FNV1A));
	if (!x58_csi_state_admit(broad_profile, "RUNTIME_POST_CSI_PROFILE"))
		return false;
	 ...
	 */
	return x58_csi_state_admit(
		pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0x50) ==
			0x160c0112u &&
		pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0x54) ==
			0x00000012u &&
		pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0x6c) ==
		...
		pci_io_read_config32(X58_VENDOR_IOH_STAGE_DEV, 0x9c) ==
			0xea000000u &&
		pci_io_read_config32(X58_VENDOR_IOH_SYRE_DEV, 0xcc) ==
			0x00000600u, "RUNTIME_POST_CSI_ENDPOINT");
}

static bool x58_minit_high_qpi_minit_is_authorized(void)
{
	return runtime.x58_init_phase_high_qpi_csi_profile &&
		runtime.x58_minit_high_qpi_minit_authorized;
}
...
static bool x58_memory_result_high_qpi_post_minit_platform_exact(void);
...
static enum x58_vendor_status csi_platform_gate(void)
{
	...
	if (runtime.x58_init_phase_high_qpi_csi_profile) {
		if (runtime.minit_returned) {
			if (!x58_memory_result_high_qpi_post_minit_platform_exact())
				return X58_VENDOR_ERR_PLATFORM_STATE;
		} else
		if (runtime.csi_returned) {
			if (!x58_minit_high_qpi_post_csi_platform_exact())
				return X58_VENDOR_ERR_PLATFORM_STATE;
		} else if (!x58_init_phase_high_qpi_platform_exact()) {
			return X58_VENDOR_ERR_PLATFORM_STATE;
		}
	...
	if (pci_io_read_config32(X58_VENDOR_MEMORY_CLOCK_DEV, 0x50) !=
	    ...
	    pci_io_read_config32(X58_VENDOR_MEMORY_CLOCK_DEV, 0x54) !=
			X58_VENDOR_MEMORY_RATIO_STATE) {
		/* Only the post-CSI/pre-MINIT phase is warning-only. */
		if (runtime.x58_init_phase_high_qpi_csi_profile && runtime.csi_returned &&
		    !runtime.minit_returned &&
		    x58_csi_state_admit(false, "RUNTIME_POST_CSI_MEMORY_CLOCK"))
			return X58_VENDOR_OK;
		return X58_VENDOR_ERR_PLATFORM_STATE;
	}
	...
	if (runtime.x58_init_phase_high_qpi_csi_profile) {
		if (runtime.minit_returned) {
			if (pciexbar_low != X58_VENDOR_PCIEXBAR_LO ||
			    !x58_memory_result_high_qpi_post_minit_platform_exact())
				return X58_VENDOR_ERR_PLATFORM_STATE;
		} else
		if (runtime.csi_returned) {
			if (pciexbar_low != X58_VENDOR_PCIEXBAR_LO ||
			    !x58_minit_high_qpi_post_csi_platform_exact())
				return X58_VENDOR_ERR_PLATFORM_STATE;
		} else if (pciexbar_low != X58_VENDOR_PCIEXBAR_LO ||
			   !x58_init_phase_high_qpi_platform_exact()) {
			return X58_VENDOR_ERR_PLATFORM_STATE;
		}
	} else if (qpi_state != X58_VENDOR_QPI_SLOW_STATE) {
		return X58_VENDOR_ERR_PLATFORM_STATE;
	...
	struct x58_vendor_runtime_info *info)
{
	...
	if (runtime.magic == X58_VENDOR_RUNTIME_MAGIC && runtime_layout_valid()) {
		...
		info->x58_init_phase_high_qpi_csi_profile =
			runtime.x58_init_phase_high_qpi_csi_profile;
		info->x58_minit_high_qpi_minit_authorized =
			runtime.x58_minit_high_qpi_minit_authorized;
		info->x58_csi_profile_pre_csi_cpu_a0 = runtime.x58_csi_profile_pre_csi_cpu_a0;
		info->x58_csi_profile_pre_csi_cpu_a0_valid =
			runtime.x58_csi_profile_pre_csi_cpu_a0_valid;
		info->vendor_status_seed = runtime.vendor_status_seed;
...
static bool x58_memory_result_high_qpi_post_minit_platform_exact(void)
{
	...
	/* Only Run2's byte-08/raw pair is eligible for a X58_MEMORY_RESULT handoff. */
	if (!runtime.x58_init_phase_high_qpi_csi_profile ||
	    !runtime.x58_minit_high_qpi_minit_authorized ||
	    ...
	    runtime.vendor_status_seed != 0 ||
	    state[X58_VENDOR_CSI_DYNAMIC_OFFSET] != 0x08 ||
	    runtime.csi_state_digest != X58_VENDOR_CSI_RAW_08_FNV1A ||
	    bytes_digest(state, X58_VENDOR_CSI_STATE_SIZE) !=
		X58_VENDOR_CSI_RAW_08_FNV1A ||
	    canonical_digest != X58_VENDOR_CSI_CANONICAL_FNV1A ||
	    runtime.minit_policy_digest != X58_VENDOR_MEMORY_RESULT_POLICY_FNV1A ||
	    bytes_digest(policy, X58_VENDOR_MINIT_POLICY_SIZE) !=
		X58_VENDOR_MEMORY_RESULT_POLICY_FNV1A ||
	    bytes_digest(workspace, X58_VENDOR_MINIT_WORKSPACE_SIZE) !=
		X58_VENDOR_MEMORY_RESULT_WORKSPACE_FNV1A ||
	    workspace[1] != 0 || workspace[2] != 0 ||
	    workspace[X58_VENDOR_MEMORY_RESULT_WORK_B3_FLAGS_OFFSET] != 0x02 ||
	    workspace[X58_VENDOR_MEMORY_RESULT_WORK_COMPLETE_OFFSET] != 0x01 ||
	    policy[X58_VENDOR_MEMORY_RESULT_POLICY_STATUS_OFFSET] != 0 ||
	...
	if (pci_io_read_config32(X58_VENDOR_SAD_DEV, 0) != X58_VENDOR_SAD_ID ||
	    ...
	    fixed_read32(0xe0000000u) != X58_VENDOR_ID ||
	    fixed_read32(0xe0000008u) != 0x06000013u)
		return false;
	...
	return pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0x50) ==
		...
		pci_io_read_config32(X58_VENDOR_MEMORY_CLOCK_DEV, 0x54) ==
			X58_VENDOR_MEMORY_RATIO_STATE &&
		pci_io_read_config32(X58_VENDOR_MEMORY_RESULT_MC_COMMON_DEV, 0x60) ==
			0x00024489u &&
		pci_io_read_config32(X58_VENDOR_MEMORY_RESULT_UNCORE_COMMON_DEV, 0xf8) ==
			0x00001545u &&
		pci_io_read_config32(X58_VENDOR_MEMORY_RESULT_CHANNEL2_ADDR_DEV, 0x48) ==
			0x000002acu &&
		pci_io_read_config32(X58_VENDOR_MEMORY_RESULT_CHANNEL2_DEV, 0x7c) ==
			0x00000003u &&
		pci_io_read_config32(X58_VENDOR_MEMORY_RESULT_CHANNEL2_DEV, 0x5c) ==
			0x00000140u;
}
	...
	struct x58_vendor_runtime_info *info)
{
	...
	if (status != X58_VENDOR_OK)
		return status;
	if (info->minit_policy_digest != X58_VENDOR_MEMORY_RESULT_POLICY_FNV1A ||
	    info->minit_policy_current_digest != X58_VENDOR_MEMORY_RESULT_POLICY_FNV1A)
	...
	uint32_t confirmation)
{
	...
	if (confirmation != X58_VENDOR_WRITE_CONFIRMATION)
	...
	if (!(x58_vendor_csi_profile_high_qpi_cpu_a0_exact(saved_pre_a0)
	      || x58_vendor_normal_stable_pre_minit_a0(saved_pre_a0)
	     ) ||
	    pre_a0_first != pre_a0_second || pre_a0_first != saved_pre_a0)
		return X58_VENDOR_ERR_PLATFORM_STATE;
	if (!x58_init_phase_high_qpi_platform_exact())
		return X58_VENDOR_ERR_PLATFORM_STATE;
	if (pci_io_read_config32(X58_VENDOR_QPI_PHY_DEV, 0xa0) != saved_pre_a0)
		return X58_VENDOR_ERR_PLATFORM_STATE;
	runtime.x58_csi_profile_pre_csi_cpu_a0 = saved_pre_a0;
	runtime.x58_csi_profile_pre_csi_cpu_a0_valid = true;

	runtime.x58_init_phase_high_qpi_csi_profile = true;
	runtime.x58_minit_high_qpi_minit_authorized = false;
	return X58_VENDOR_OK;
}
	...
	uint32_t confirmation)
{
	...
	runtime.x58_minit_high_qpi_minit_authorized = true;
	return X58_VENDOR_OK;
}
	...
	uint8_t vendor_status_seed, uint32_t confirmation)
{
	...
	runtime.vendor_status_seed = vendor_status_seed;
	...
	struct x58_vendor_call_result *result)
{
	...
	vendor_call(X58_VENDOR_CSI_WRAPPER_ENTRY, 0, 0,
		runtime.vendor_status_seed, 3, result);
	...
	uint32_t confirmation)
{
	...
	if (runtime.minit_call_attempted)
		return X58_VENDOR_ERR_ORDER;
	if (runtime.x58_init_phase_high_qpi_csi_profile
	    && !x58_minit_high_qpi_minit_is_authorized()
	   )
		return X58_VENDOR_ERR_PLATFORM_STATE;
