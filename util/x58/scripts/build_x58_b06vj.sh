#!/usr/bin/env bash
# Build and locally compose the exact MSI X58 Pro-E B06VJ experiment.
...
config="${repo_root}/configs/x58-pro-e-b06vj.config"
public_rom="${repo_root}/builds/experimental/msi-x58-pro-e-b06vj-coreboot-base-4MiB.rom"
local_dir="${repo_root}/blobs-local/msi-x58-pro-e/b06vj"
vendor_rom="${X58_B06VJ_VENDOR_ROM:-${repo_root}/blobs-local/msi-x58-pro-e/7522v8F/A7522IMS.8F0}"
jobs="${X58_B06VJ_JOBS:-8}"
epoch="${X58_B06VJ_SOURCE_DATE_EPOCH:-1788602400}"
temp_dir="$(mktemp -d /tmp/x58-b06vj-build.XXXXXX)"
...
if [[ ! "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
	echo "X58_B06VJ_JOBS must be a positive integer" >&2
	exit 1
fi
...
python3 -m unittest \
	...
	tests.test_x58_b06vj_source_contract -v

install_exact "${coreboot_dir}/build/coreboot.rom" "${public_rom}"

unpatched="${temp_dir}/msi-x58-pro-e-b06vj-unpatched-4MiB.rom"
deterministic="${temp_dir}/msi-x58-pro-e-b06vj-deterministic-4MiB.rom"
full_chip="${temp_dir}/msi-x58-pro-e-b06vj-deterministic-w25q128-16MiB.rom"
...
install_exact "${unpatched}" \
	"${local_dir}/msi-x58-pro-e-b06vj-unpatched-4MiB.rom"
install_exact "${deterministic}" \
	"${local_dir}/msi-x58-pro-e-b06vj-deterministic-4MiB.rom"
install_exact "${full_chip}" \
	"${local_dir}/msi-x58-pro-e-b06vj-deterministic-w25q128-16MiB.rom"
