#!/usr/bin/env bash
# Build and locally compose the MSI X58 Pro-E B06VL broad hard-gated attempt.
...
config="${repo_root}/configs/x58-pro-e-b06vl.config"
public_rom="${repo_root}/builds/experimental/msi-x58-pro-e-b06vl-coreboot-base-4MiB.rom"
local_dir="${repo_root}/blobs-local/msi-x58-pro-e/b06vl"
vendor_rom="${X58_B06VL_VENDOR_ROM:-${repo_root}/blobs-local/msi-x58-pro-e/7522v8F/A7522IMS.8F0}"
jobs="${X58_B06VL_JOBS:-8}"
epoch="${X58_B06VL_SOURCE_DATE_EPOCH:-1788602400}"
temp_dir="$(mktemp -d /tmp/x58-b06vl-build.XXXXXX)"
...
if [[ ! "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
	echo "X58_B06VL_JOBS must be a positive integer" >&2
	exit 1
fi
...
python3 -m unittest \
	...
	tests.test_x58_b06vl_source_contract -v

install_exact "${coreboot_dir}/build/coreboot.rom" "${public_rom}"

unpatched="${temp_dir}/msi-x58-pro-e-b06vl-unpatched-4MiB.rom"
deterministic="${temp_dir}/msi-x58-pro-e-b06vl-deterministic-4MiB.rom"
full_chip="${temp_dir}/msi-x58-pro-e-b06vl-deterministic-w25q128-16MiB.rom"
...
install_exact "${unpatched}" \
	"${local_dir}/msi-x58-pro-e-b06vl-unpatched-4MiB.rom"
install_exact "${deterministic}" \
	"${local_dir}/msi-x58-pro-e-b06vl-deterministic-4MiB.rom"
install_exact "${full_chip}" \
	"${local_dir}/msi-x58-pro-e-b06vl-deterministic-w25q128-16MiB.rom"
