#!/usr/bin/env bash
# Build and locally compose the MSI X58 Pro-E B06VN automatic first-boot image.
...
config="${repo_root}/configs/x58-pro-e-b06vn.config"
public_rom="${repo_root}/builds/experimental/msi-x58-pro-e-b06vn-coreboot-base-4MiB.rom"
local_dir="${repo_root}/blobs-local/msi-x58-pro-e/b06vn"
vendor_rom="${X58_B06VN_VENDOR_ROM:-${repo_root}/blobs-local/msi-x58-pro-e/7522v8F/A7522IMS.8F0}"
jobs="${X58_B06VN_JOBS:-8}"
epoch="${X58_B06VN_SOURCE_DATE_EPOCH:-1788602400}"
...
temp_dir="$(mktemp -d /tmp/x58-b06vn-build.XXXXXX)"
...
if [[ ! "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
	echo "X58_B06VN_JOBS must be a positive integer" >&2
	exit 1
fi
...
if grep -Eq 'pci1002,[[:xdigit:]]{4}\.rom' "${cbfs_listing}"; then
	echo "B06VN must not embed an AMD VGA option ROM" >&2
	exit 1
fi
...
python3 -m unittest \
	...
	tests.test_x58_b06vn_source_contract -v
...
if ! cmp -s -- "${first_ipxe_rom}" "${ipxe_rom}"; then
	echo "B06VN iPXE ROM differs between clean builds" >&2
	exit 1
fi
if ! cmp -s -- "${first_rom}" "${second_rom}"; then
	echo "B06VN clean builds are not byte-identical" >&2
	exit 1
fi

install_exact "${second_rom}" "${public_rom}"

unpatched="${temp_dir}/msi-x58-pro-e-b06vn-unpatched-4MiB.rom"
deterministic="${temp_dir}/msi-x58-pro-e-b06vn-deterministic-4MiB.rom"
full_chip="${temp_dir}/msi-x58-pro-e-b06vn-deterministic-w25q128-16MiB.rom"
...
install_exact "${unpatched}" \
	"${local_dir}/msi-x58-pro-e-b06vn-unpatched-4MiB.rom"
install_exact "${deterministic}" \
	"${local_dir}/msi-x58-pro-e-b06vn-deterministic-4MiB.rom"
install_exact "${full_chip}" \
	"${local_dir}/msi-x58-pro-e-b06vn-deterministic-w25q128-16MiB.rom"
