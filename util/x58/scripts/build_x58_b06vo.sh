#!/usr/bin/env bash
# Build and locally compose the MSI X58 Pro-E B06VO IOHBUSNO probe image.
...
config="${repo_root}/configs/x58-pro-e-b06vo.config"
public_rom="${repo_root}/builds/experimental/msi-x58-pro-e-b06vo-coreboot-base-4MiB.rom"
local_dir="${repo_root}/blobs-local/msi-x58-pro-e/b06vo"
vendor_rom="${X58_B06VO_VENDOR_ROM:-${repo_root}/blobs-local/msi-x58-pro-e/7522v8F/A7522IMS.8F0}"
jobs="${X58_B06VO_JOBS:-8}"
epoch="${X58_B06VO_SOURCE_DATE_EPOCH:-1788688800}"
...
temp_dir="$(mktemp -d /tmp/x58-b06vo-build.XXXXXX)"
...
if [[ ! "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
	echo "X58_B06VO_JOBS must be a positive integer" >&2
	exit 1
fi
...
if grep -Eq 'pci1002,[[:xdigit:]]{4}\.rom' "${cbfs_listing}"; then
	echo "B06VO must not embed an AMD VGA option ROM" >&2
	exit 1
fi
...
python3 -m unittest \
	...
	tests.test_x58_b06vo_source_contract -v
...
if ! cmp -s -- "${first_ipxe_rom}" "${ipxe_rom}"; then
	echo "B06VO iPXE ROM differs between clean builds" >&2
	exit 1
fi
if ! cmp -s -- "${first_rom}" "${second_rom}"; then
	echo "B06VO clean builds are not byte-identical" >&2
	exit 1
fi

install_exact "${second_rom}" "${public_rom}"

unpatched="${temp_dir}/msi-x58-pro-e-b06vo-unpatched-4MiB.rom"
deterministic="${temp_dir}/msi-x58-pro-e-b06vo-deterministic-4MiB.rom"
full_chip="${temp_dir}/msi-x58-pro-e-b06vo-deterministic-w25q128-16MiB.rom"
...
install_exact "${unpatched}" \
	"${local_dir}/msi-x58-pro-e-b06vo-unpatched-4MiB.rom"
install_exact "${deterministic}" \
	"${local_dir}/msi-x58-pro-e-b06vo-deterministic-4MiB.rom"
install_exact "${full_chip}" \
	"${local_dir}/msi-x58-pro-e-b06vo-deterministic-w25q128-16MiB.rom"
