#!/usr/bin/env bash
# Build and locally compose the MSI X58 Pro-E B06VQ six-field ICH10 baseline.
...
config="${repo_root}/configs/x58-pro-e-b06vq-ichbase1.config"
public_rom="${repo_root}/builds/experimental/msi-x58-pro-e-b06vq-ichbase1-coreboot-base-4MiB.rom"
local_dir="${repo_root}/blobs-local/msi-x58-pro-e/b06vq-ichbase1"
vendor_rom="${X58_B06VQ_ICHBASE1_VENDOR_ROM:-${repo_root}/blobs-local/msi-x58-pro-e/7522v8F/A7522IMS.8F0}"
jobs="${X58_B06VQ_ICHBASE1_JOBS:-8}"
epoch="${X58_B06VQ_ICHBASE1_SOURCE_DATE_EPOCH:-1788699600}"
...
temp_dir="$(mktemp -d /tmp/x58-b06vq-ichbase1-build.XXXXXX)"
...
if [[ ! "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
	echo "X58_B06VQ_ICHBASE1_JOBS must be a positive integer" >&2
	exit 1
fi
...
if grep -Fq 'etc/usb-time-sigatt' "${cbfs_listing}"; then
	echo "B06VQ-ICHBASE1 must not inherit the USBTRACE1 attach delay" >&2
	exit 1
fi
if grep -Eq 'pci1002,[[:xdigit:]]{4}\.rom' "${cbfs_listing}"; then
	echo "B06VQ-ICHBASE1 must not embed an AMD VGA option ROM" >&2
	exit 1
fi
...
if ! cmp -s -- "${first_ipxe_rom}" "${ipxe_rom}"; then
	echo "B06VQ-ICHBASE1 iPXE ROM differs between clean builds" >&2
	exit 1
fi
if ! cmp -s -- "${first_rom}" "${second_rom}"; then
	echo "B06VQ-ICHBASE1 clean builds are not byte-identical" >&2
	exit 1
fi

install_exact "${second_rom}" "${public_rom}"

unpatched="${temp_dir}/msi-x58-pro-e-b06vq-ichbase1-unpatched-4MiB.rom"
deterministic="${temp_dir}/msi-x58-pro-e-b06vq-ichbase1-deterministic-4MiB.rom"
full_chip="${temp_dir}/msi-x58-pro-e-b06vq-ichbase1-deterministic-w25q128-16MiB.rom"
...
install_exact "${unpatched}" \
	"${local_dir}/msi-x58-pro-e-b06vq-ichbase1-unpatched-4MiB.rom"
install_exact "${deterministic}" \
	"${local_dir}/msi-x58-pro-e-b06vq-ichbase1-deterministic-4MiB.rom"
install_exact "${full_chip}" \
	"${local_dir}/msi-x58-pro-e-b06vq-ichbase1-deterministic-w25q128-16MiB.rom"
