#!/usr/bin/env bash
# Build and locally compose the MSI X58 Pro-E B06VT SATA clock-field image.
...
config="${repo_root}/configs/x58-pro-e-b06vt.config"
public_rom="${repo_root}/builds/experimental/msi-x58-pro-e-b06vt-coreboot-base-4MiB.rom"
local_dir="${repo_root}/blobs-local/msi-x58-pro-e/b06vt"
vendor_rom="${X58_B06VT_VENDOR_ROM:-${repo_root}/blobs-local/msi-x58-pro-e/7522v8F/A7522IMS.8F0}"
jobs="${X58_B06VT_JOBS:-8}"
epoch="${X58_B06VT_SOURCE_DATE_EPOCH:-1788696000}"
...
temp_dir="$(mktemp -d /tmp/x58-b06vt-build.XXXXXX)"
...
if [[ ! "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
	echo "X58_B06VT_JOBS must be a positive integer" >&2
	exit 1
fi
...
if grep -Eq 'pci1002,[[:xdigit:]]{4}\.rom' "${cbfs_listing}"; then
	echo "B06VT must not embed an AMD VGA option ROM" >&2
	exit 1
fi
...
python3 -m unittest \
	...
	tests.test_x58_b06vt_source_contract -v

build_once "${second_rom}"
verify_prompted_ipxe_source
verify_ipxe_rom
cmp -s -- "${first_ipxe_rom}" "${ipxe_rom}" || {
	echo "B06VT iPXE ROM differs between clean builds" >&2
	exit 1
}
cmp -s -- "${first_rom}" "${second_rom}" || {
	echo "B06VT clean builds are not byte-identical" >&2
	exit 1
}

install_exact "${second_rom}" "${public_rom}"

unpatched="${temp_dir}/msi-x58-pro-e-b06vt-unpatched-4MiB.rom"
deterministic="${temp_dir}/msi-x58-pro-e-b06vt-deterministic-4MiB.rom"
full_chip="${temp_dir}/msi-x58-pro-e-b06vt-deterministic-w25q128-16MiB.rom"
...
install_exact "${unpatched}" "${local_dir}/msi-x58-pro-e-b06vt-unpatched-4MiB.rom"
install_exact "${deterministic}" "${local_dir}/msi-x58-pro-e-b06vt-deterministic-4MiB.rom"
install_exact "${full_chip}" "${local_dir}/msi-x58-pro-e-b06vt-deterministic-w25q128-16MiB.rom"

sha256sum "${public_rom}" \
	"${local_dir}/msi-x58-pro-e-b06vt-unpatched-4MiB.rom" \
	"${local_dir}/msi-x58-pro-e-b06vt-deterministic-4MiB.rom" \
	"${local_dir}/msi-x58-pro-e-b06vt-deterministic-w25q128-16MiB.rom" \
	"${ipxe_rom}"
